...
function transformData(data) {
  ...
  for (var i = 0; i < data[0].length; i++) {
    ...
    for (var j = 0; j < data.length; j++) {
      sumOfDigits += data[j][i];
    }

    transformedData.push(sumOfDigits);
  }

  var finalSum = 0;
  var endOfArray = transformedData.length - 1;
  for (i = endOfArray; i >= 0; i--) {
    finalSum += (transformedData[i] * Math.pow(10, endOfArray - i));
    console.log(finalSum);
  }

  // Get first 10 digits
  var finalSumAsString = finalSum.toString();

  return finalSumAsString.charAt(0) + finalSumAsString.slice(2, 11);
}
